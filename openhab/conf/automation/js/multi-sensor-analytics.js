...
function lagAnalysis(left, right) {
  if (left.length < MIN_SAMPLES.lag || right.length < MIN_SAMPLES.lag) { return { lag: 0, best: null, base: null }; }
  let bestLag = 0;
...
function leadText(pair, analysis) {
  if (analysis.best === null) { return 'Collecting samples'; }
...
function causeText(pair, analysis) {
  if (analysis.best === null || analysis.base === null) { return 'Collecting samples'; }
...
function updateMatrix() {
  ...
  if (available < MIN_SAMPLES.matrix) {
...
function updateMetrics() {
  ...
  PAIRS.forEach(function (pair) {
    ...
    const correlation = left.length >= MIN_SAMPLES.correlation && right.length >= MIN_SAMPLES.correlation ? pearson(left, right) : null;
  ...
  updateMatrix();
  postText('Analytics_Status', temperatureCorrelationWindow < WINDOWS.correlation ? 'Warm-up ' + temperatureCorrelationWindow + '/' + WINDOWS.correlation + ' samples (corr 3+, matrix 8+, lag 12+, trend 10+)' : 'Ready: full analytics window active');
